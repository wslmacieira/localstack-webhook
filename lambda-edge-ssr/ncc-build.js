...
const options = {
  ...
  externals: ['@codegenie/serverless-express'],
  minify: true,
  sourceMap: false,
  watch: false // default
};
...
async function build(file) {
  ...
  await mkdirp(resolve(DIST_DIR));
  write(resolve(DIST_DIR, name + ".js"), code);
}
...
async function main() {
  ...
  const files = await glob(resolve(__dirname, "src/*lambda.js"));
